class ToolManager {
    ...
    setTool(tool, layerManager) {
        ...
        if(tool === 'layer-tool') {
            this.openLayersWindow(layerManager);
        }
        else {
    ...
    openLayersWindow(layerManager) {
        let layersHTML = [];
        layersHTML.push(this.getLayersTitleHTML());
        for(let i = 0; i < layerManager.numOfLayers; i++) {
            layersHTML.push('<div class="layer">');
            layersHTML.push(this.getEyeIconHTML());
            layersHTML.push(`<p>Layer ${i + 1}</p>`);
            layersHTML.push(this.getTrashIconHTML());
            layersHTML.push('</div>');
        }
    ...
    getEyeIconHTML() {
        const html =    '<svg class="layers-tool" viewBox="0 0 24 24">' +
    ...
    getTrashIconHTML() {
        const html =    '<svg id="layer-trash" class="layers-tool" viewBox="0 0 24 24" fill="none">' +
